...
public class mainController {
    @GetMapping("/")
    public String main(){
        return "redirect:index"; // forward or redirect
        // forward : 내부(seission, model) 변수값을 가지고 페이지 이동 ( 내부 URL로 이동시 )
        // redirect : 내부 변수값을 버리고 ㅔ이지 이동 ( 타 사이트로 이동시 )
    }
    @GetMapping("/index")
    ...
    // FrontEnd - HTML에서 서버로 데이터를 전송하는 방법
    // HTML Form 태그 : GET/POST 방식으로 submit한다.
